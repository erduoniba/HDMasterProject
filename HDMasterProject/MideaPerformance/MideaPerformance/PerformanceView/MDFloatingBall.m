...
#pragma mark - MDFloatingBallWindow

@interface MDFloatingBallWindow : UIWindow
@end

@implementation MDFloatingBallWindow

- (void)becomeKeyWindow {
    [[MDFloatingBallManager shareManager].appKeyWindow makeKeyWindow];
}

- (BOOL)pointInside:(CGPoint)point withEvent:(UIEvent *)event {
    __block MDFloatingBall *floatingBall = nil;
    [self.subviews enumerateObjectsUsingBlock:^(__kindof UIView * _Nonnull obj, NSUInteger idx, BOOL * _Nonnull stop) {
        if ([obj isKindOfClass:[MDFloatingBall class]]) {
            floatingBall = (MDFloatingBall *)obj;
            *stop = YES;
        }
    }];
...
- (void)mis_addSubview:(UIView *)subview {
    ...
    if ([MDFloatingBallManager shareManager].canRuntime) {
        if ([[MDFloatingBallManager shareManager].superView isEqual:self]) {
            [self.subviews enumerateObjectsUsingBlock:^(UIView * obj, NSUInteger idx, BOOL * _Nonnull stop) {
                if ([obj isKindOfClass:[MDFloatingBall class]]) {
                    [self insertSubview:subview belowSubview:(MDFloatingBall *)obj];
                }
            }];
        }
    }
...
#pragma mark - MDFloatingBall

@interface MDFloatingBall()
@property (nonatomic, assign) CGPoint centerOffset;
@property (nonatomic,   copy) MISEdgeRetractConfig(^edgeRetractConfigHander)(void);
...
static const NSInteger minUpDownLimits = 60 * 1.5f;   // MDFloatingBallEdgePolicyAllEdge 下，悬浮球到达一个界限开始自动靠近上下边缘
...
@implementation MDFloatingBall

#pragma mark - Life Cycle

- (void)dealloc {
    MISLog(@"MDFloatingBall dealloc");
    [MDFloatingBallManager shareManager].canRuntime = NO;
    [MDFloatingBallManager shareManager].superView = nil;
}
...
- (instancetype)initWithFrame:(CGRect)frame inSpecifiedView:(UIView *)specifiedView effectiveEdgeInsets:(UIEdgeInsets)effectiveEdgeInsets {
    ...
    if (self) {
        ...
        _autoCloseEdge = NO;
        _autoEdgeRetract = NO;
        _edgePolicy = MDFloatingBallEdgePolicyAllEdge;
...
- (void)configSpecifiedView:(UIView *)specifiedView {
    if (specifiedView) {
        ...
        UIWindow *window = [[MDFloatingBallWindow alloc] initWithFrame:[UIScreen mainScreen].bounds];
...
//        [window makeKeyAndVisible];
    ...
    // setup ball manager
    [MDFloatingBallManager shareManager].canRuntime = YES;
    [MDFloatingBallManager shareManager].superView = specifiedView;
}
...
- (CGPoint)calculatePoisitionWithEndOffset:(CGPoint)offset {
    ...
    if (MDFloatingBallEdgePolicyLeftRight == self.edgePolicy) {
        // 左右
    ...
    else if (MDFloatingBallEdgePolicyUpDown == self.edgePolicy) {
    ...
    else if (MDFloatingBallEdgePolicyAllEdge == self.edgePolicy) {
...
- (void)autoEdgeRetractDuration:(NSTimeInterval)duration edgeRetractConfigHander:(MISEdgeRetractConfig (^)(void))edgeRetractConfigHander {
...
- (void)setContent:(id)content contentType:(MDFloatingBallContentType)contentType {
    BOOL notUnknowType = (MDFloatingBallContentTypeCustomView == contentType) || (MDFloatingBallContentTypeImage == contentType) || (MDFloatingBallContentTypeText == contentType);
    NSAssert(notUnknowType, @"can't set ball content with an unknow content type");
    
    [self.ballCustomView removeFromSuperview];
    if (MDFloatingBallContentTypeImage == contentType) {
    ...
    else if (MDFloatingBallContentTypeText == contentType) {
    ...
    else if (MDFloatingBallContentTypeCustomView == contentType) {
