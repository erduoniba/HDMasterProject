...
- (void)setImageFlipped:(BOOL)flipped
{
    if (self.isSideView) {
        ...
        if (flipped) {
            _textLb.text = @"释放查看楼盘详情";
        }
        else {
            _textLb.text = @"滑动查看楼盘详情";
        }
...
- (void)layoutSubviews
{
    ...
    if (self.isSideView)
        self.arrowImage.position = CGPointMake(visibleLeft - kSidePullViewWidth + 25.0, self.frame.size.height / 4.0 + self.arrowImage.frame.size.height);
    else
...
- (id)initWithScrollView:(UIScrollView *)scroll position:(AllAroundPullViewPosition)position
{
    ...
    if ((self = [super initWithFrame:frame])) {
        ...
        UIImage *arrow = [UIImage imageNamed:@"fdd_logo_refresh"];
        ...
        if (self.isSideView) {
            ...
            arrowAndActivityFrame = CGRectMake(sideArrowPositionOffset - kSidePullViewWidth + 20.0, (self.frame.size.height - arrow.size.height) / 2, arrow.size.width, arrow.size.height);
        } else {
        ...
        _textLb = [[UILabel alloc] initWithFrame:CGRectMake(self.arrowImage.frame.size.width + self.arrowImage.frame.origin.x + 20, (self.frame.size.height - 200) / 2 - 85, 14, 200)];
        _textLb.text = @"滑动查看楼盘详情";
